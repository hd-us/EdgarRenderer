...
import IoManager, Utils, EdgarRenderer
...
def analyzeFactsInCubes(filing): # void
    ...
    for v in sorted(messages,key=lambda x: x[3]): # Messages should be ordered by line number in source doc.
        (f,cube,atts,line) = v
        controller.logWarn(("Fact {} in context {} "
                            + "in ''{}'' has an HTML anchor ({}) at line {} that "
                           ...
                           .format(f.qname,f.contextID,cube.shortName,atts,line))
...
class Summary(object):
    ...
    def appendSummaryHeader(self):
        ...
        SubElement(rootETree, 'Version').text = EdgarRenderer.VERSION 
